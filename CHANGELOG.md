...
All notable changes are documented here. NetLang follows Semantic Versioning for release artifacts and versions its machine contracts independently.
...
- Agent-oriented CLI with stdin, `netlang.cli.v1` JSON, stable diagnostics/exit codes and safe artifact writes.
...
This is the first versioned release, so there is no earlier public schema to migrate. Prototype users should note that `render` is now functional, editable export uses `netlang export --target ...`, compile reports are `netlang.compile.v3`, and browser share links require `netlang.share.v1`; unknown older payloads fail closed.
