...
- [ ] SQ-9 — Kabul edilen scorecard/metric/hash/screenshot baseline'larını kilitle ve canonical local/remote kalite kapılarını çalıştır. _2026-08-14 local canonical `scripts/verify.ps1` PASS: fmt, Clippy, bütün Rust testleri (16 schematic contract), release/WASM, 10 unit + 11 Chromium E2E, audit, agent replay, clean release smoke ve kurulu KiCad/LTspice smoke. Candidate hash/scorecard regression'ları kilitli; bu commit'in remote CI sonucu bekleniyor._
...
**Şema remediation durumu (2026-08-14):** 2026-08-13 candidate'ı owner görsel audit'inde rail/GND anchor'ları, component text yerleşimi, gereksiz routing detour'ları ve supply kompozisyonu nedeniyle reddedildi. Bulgular ortak Schematic IR/layout motorunda yeniden işlendi; on üç devrelik Core/Web candidate ve SVG/PNG/PDF/KiCad/LTspice parity kanıtı hazırlandı. Local canonical full gate PASS; SQ-9'un remote CI ayağı ve proje sahibi Web onayı SQ-10 açık olduğundan yeni candidate `docs/evals/schematic-quality-candidate-2026-08-14.md` içinde henüz accepted golden değildir. Eski kanıt geçmiş kayıt olarak korunur.
