...
- [ ] SQ-9 — Kabul edilen scorecard/metric/hash/screenshot baseline'larını kilitle ve canonical local/remote kalite kapılarını çalıştır. _2026-08-14 local canonical `scripts/verify.ps1` PASS: fmt, Clippy, bütün Rust testleri (17 schematic contract), release/WASM, 10 unit + 11 Chromium E2E, audit, agent replay, clean release smoke ve kurulu KiCad/LTspice smoke. Candidate hash/scorecard regression'ları kilitli; bu commit'in remote CI sonucu bekleniyor._
...
**Şema remediation durumu (2026-08-14):** 2026-08-13 candidate'ı owner görsel audit'inde rail/GND anchor'ları, component text yerleşimi, gereksiz routing detour'ları ve supply kompozisyonu nedeniyle reddedildi. Bulgular ortak Schematic IR/layout motorunda yeniden işlendi; on üç devrelik Core/Web candidate ve SVG/PNG/PDF/KiCad/LTspice parity kanıtı hazırlandı. İkinci owner review routing ve genel görünümü olumlu buldu; bildirdiği uzak reference/value alanları routing'e dokunmayan sub-grid yerleşimle yaklaştırıldı, linked Web highlight eklendi ve BJT emitter okları temizlendi. On üç Core görseli ile gerçek Web etkileşimi yeniden agent-reviewed durumdadır. SQ-9'un remote CI ayağı ve son owner kabulü beklenen SQ-10 nedeniyle candidate henüz accepted golden değildir. Eski kanıt geçmiş kayıt olarak korunur.
