...
- [ ] Compare the same identified model and agent harness with Kessetsu versus direct Ngspice plus scripts and the relevant EDA workflow. Give both equal specifications, tool access, model data, and run budgets; allow baseline automation. Start with one accessible model, repeat each task at least three times, then check whether conclusions transfer to another available model. Record unavailable access rather than inventing runs. _U1 and U2 checkpoints completed 2026-09-12 with `gpt-5.6-sol`/medium; both arms passed 3/3 electrical evaluations in each family. U1 favored Kessetsu elapsed time and U2 favored direct Ngspice. Kessetsu established repeatable automatic artifact/interoperability value: 3/3 full U1 export sets and 2/3 full U2 sets, with all generated editable exports opening in their target applications. Direct produced hand-authored U1 SVGs and text-only U2 schematic descriptions. Evidence: `docs/evals/unseen-design-u1-comparison-2026-09-12.md` and `docs/evals/unseen-design-u2-comparison-2026-09-12.md`. Continue through U3–U6 before closing the gate._
...
- [ ] Save prompts, exact available model identifiers/settings, sources, tool traces, failures, human interventions, elapsed time, usage/cost when available, and final artifacts. Report successful designs, false passes, unsupported cases, and artifact usability separately. Existing fixture replay remains a regression test. _U1 and U2 records are complete locally under `.artifacts/agent-comparison-v1/`; their committed reports record scope, hashes, usage, corrections/exclusions, and artifact usability. Repeat for U3–U6 before checking this item._
- [ ] Review fresh schematic PNGs and open editable exports from successful unseen tasks; a corpus hash alone cannot establish readable new layouts. _U1 completed 2026-09-12: three fresh Kessetsu PNGs and all editable exports passed review. U2 completed 2026-09-12: both generated Kessetsu PNGs were visually inspected; both KiCad exports opened/netlisted with all nine references, zero ERC errors, and nine known `lib_symbol_issues` warnings; both LTspice exports opened/netlisted with all references. Kessetsu U2 attempt 2 omitted exports and is recorded as 2/3 artifact delivery. Direct U2 produced text descriptions only. Continue for later tasks._
...
**4.6V — Extend the fresh-context comparison to U3.**

U1 and U2 are complete. Both arms achieved 3/3 electrical success in each family; Kessetsu's automatic artifact/interoperability benefit repeated, while U2 exposed 2/3 rather than perfect artifact-follow-through and a slower elapsed-time median. The U3 v2 harness and exact frozen 2N3904 distribution are ready. On 2026-09-12, one pre-inference Windows PowerShell path failure and one Kessetsu inference interrupted by the owner's Codex usage limit were preserved under `.artifacts/agent-comparison-v1/invalid-runs/`; neither is scored. Rerun U3 attempt 1 unchanged when access resets, inspect the first pair, then complete three attempts per arm if sound. Keep `gpt-5.6-sol` at medium reasoning; do not substitute an unauthorized model or API. Preserve every failure and version any post-candidate protocol correction. Owner-facing interface review and release closure remain after the comparison.
