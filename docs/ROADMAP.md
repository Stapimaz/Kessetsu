...
**İlk remote yayın kapısı kanıtı (2026-08-13):** Billing engeli kalktı ve `31727439405` numaralı dört-OS release matrisi audit dahil bütünüyle PASS verdi. Ayrı Ubuntu CI run `31727404107`, PATH'ten bulunan Ngspice çalışabildiği halde provenance yolunun absolute olmaması nedeniyle agent benchmark'ında fail ederek gerçek bir taşınabilirlik kusuru yakaladı. Discovery bundled/explicit/PATH executable'larını canonical absolute path'e çözecek şekilde düzeltildi; yerel Clippy, runner/benchmark ve ikinci canonical tam kapı PASS. Follow-up remote CI kanıtı bekleniyor.
