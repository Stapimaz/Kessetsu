...
**4.7 — Prove the final release candidate, then execute the owner-controlled publication transaction.**

The product, owner-facing workspace, local document flow, v1.0.0 version contract, newcomer package, and `kessetsu.com` static-site inputs are complete. Run the full canonical gate on the final candidate, push it, and require a green remote Linux CI run. Repository visibility, DNS/Pages custom-domain activation, the final tag, GitHub Release, production deployment and rollback proof remain deliberately unexecuted until the owner begins the public/domain transaction. Do not claim Phase 4 publication complete before those external steps pass.
