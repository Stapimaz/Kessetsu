...
- [ ] Compare the same identified model and agent harness with Kessetsu versus direct Ngspice plus scripts and the relevant EDA workflow. Give both equal specifications, tool access, model data, and run budgets; allow baseline automation. Start with one accessible model, repeat each task at least three times, then check whether conclusions transfer to another available model. Record unavailable access rather than inventing runs. _U1 checkpoint completed 2026-09-12 with `gpt-5.6-sol`/medium: both arms passed 3/3 electrical evaluations. Kessetsu's median elapsed time was 139.899 s versus 187.333 s direct, while Kessetsu used more tool calls and total input tokens. Kessetsu produced 3/3 visually reviewed PNGs plus SVG/KiCad/LTspice; all editable exports opened in their target applications. Direct produced hand-authored SVG only. Full evidence and exclusions: `docs/evals/unseen-design-u1-comparison-2026-09-12.md`. This is one task family and does not close the gate._
...
- [ ] Save prompts, exact available model identifiers/settings, sources, tool traces, failures, human interventions, elapsed time, usage/cost when available, and final artifacts. Report successful designs, false passes, unsupported cases, and artifact usability separately. Existing fixture replay remains a regression test. _U1 records are complete locally under `.artifacts/agent-comparison-v1/`; the committed report records scope, hashes, aggregate usage, invalid/calibration runs, and the evaluator correction. Repeat for U2–U6 before checking this item._
- [ ] Review fresh schematic PNGs and open editable exports from successful unseen tasks; a corpus hash alone cannot establish readable new layouts. _U1 completed 2026-09-12: three fresh Kessetsu PNGs were visually inspected; all three KiCad exports opened/netlisted with zero ERC errors and only four known `lib_symbol_issues` warnings; all three LTspice exports opened/netlisted successfully. Direct SVG files were XML-valid but could not be visually reviewed because no browser connection was available, and are not claimed as EDA-verified. Continue for later tasks._
...
**4.6V — Extend the fresh-context comparison to U2.**

The U1 checkpoint is complete and shows repeatable artifact/interoperability benefit but equal 3/3 electrical success. Extend the isolated runner and equivalent arm documentation to frozen task U2 without exposing evaluator code or prior candidates. Keep `gpt-5.6-sol` at medium reasoning, run one paired attempt first, inspect quota/evidence integrity, then complete three attempts per arm if the checkpoint remains sound. Preserve every failure and version any post-candidate protocol correction. Owner-facing interface review and release closure remain after the comparison.
