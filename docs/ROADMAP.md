...
- [x] Normalize the global Run, Export, and Share controls and make the unsaved-document state truthful. _Completed 2026-09-15: all three desktop actions share a regression-checked 76 × 30 px geometry while Run retains primary emphasis. Native file-handle browsers now provide in-place Save, Save As, and `Ctrl+S`/`Ctrl+Shift+S`; unsupported browsers retain a portable download fallback. Only a successful write/download clears the dirty marker, cancellation and errors preserve it, opened native files retain their handle, and circuit naming remains stable after editing the default example. Unit and Chromium lifecycle regressions cover handle reuse, repicking, cancellation, fallback downloads, dirty-state transitions, and control geometry; refreshed workspace screenshots were inspected. The full canonical gate passed locally with 15 Web unit tests, all 18 Chromium scenarios, zero npm vulnerabilities, and the existing two documented informational RustSec maintenance warnings._
...
- [x] Complete the Web document lifecycle: new/open/save `.kess`, versioned local draft recovery, explicit document naming, and name-derived source/export filenames without server storage. _Completed 2026-09-14 and hardened 2026-09-15: `File` owns guarded New/Open/Save/Save As/Rename flows; native file-handle browsers write in place while other browsers use a portable download fallback; only successful persistence clears the dirty marker. Local files and every Core export use deterministic name-derived filenames; stale share fragments are left when editing or replacing their source; and a bounded `kessetsu.web-draft.v1` record restores unsaved browser work without an account or upload. Contract unit and Chromium lifecycle coverage protect handle reuse, explicit Save As, cancellation, fallback downloads, and dirty-state transitions; the revised desktop workspace screenshot was inspected._
...
- [x] Restore and rerun the current full verification gate before release. _Final-candidate evidence updated 2026-09-15: branded commit `cca5123` passed the full canonical `scripts/verify.ps1` locally and remote Linux CI run `34904319476`. Coverage included version/release-note contracts, Rust/WASM, Web lint/13 unit tests/build, payload budgets, runtime/deployment audits, all 17 Chromium E2E, dependency/license/artifact audits, replay and evaluator contracts, the v1.0.0 Windows package plus clean real-Ngspice 12/12 simulation smoke, and installed KiCad/LTspice smoke. The optional user-owned TI OPA197 executions were not bound in this shell and skipped; the earlier exact-hash run and recorded U6 follow-up remain preserved. Npm audit reports zero vulnerabilities. Two documented transitive RustSec maintenance warnings and the characterized embedded-symbol KiCad warnings remain; this is not a claim of warning-free EDA interoperability or hardware validation._
...
**4.7 — Confirm the latest private candidate remotely, then execute the owner-controlled publication transaction.**

The v1.0.0 product implementation is current through `700b806`, including the owner-requested Save/Save As and header-control hardening. It passed the full canonical gate locally twice on 2026-09-15, with 15 Web unit tests and all 18 Chromium scenarios. Earlier remote run `34950853751` failed only because its optical wordmark assertion allowed less than `0.35 px` drift while Linux Chromium measured `0.4296875 px`; the accepted logo/CSS was not changed, and the cross-platform regression now allows at most half a pixel. The latest `main` CI must still pass before publication. Repository visibility, DNS/Pages custom-domain activation, the final tag, GitHub Release, production deployment verification, and rollback proof remain deliberately unexecuted until the owner begins the public/domain transaction. Follow the ordered runbook in `docs/release.md`; do not claim Phase 4 publication complete before the latest CI and those external steps pass.
