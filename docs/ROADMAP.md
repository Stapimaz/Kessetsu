...
**4.7 — Execute the owner-controlled publication transaction.**

The private v1.0.0 candidate is complete. Branded candidate `cca5123` passed both the local canonical gate and remote Linux CI run `34904319476`. Final owner-requested wordmark sizing candidate `484c62d` passed the full local canonical gate; remote Linux CI run `34950853751` is queued, superseding and cancelling the intermediate alignment-only run `34949894922`. Repository visibility, DNS/Pages custom-domain activation, the final tag, GitHub Release, production deployment verification, and rollback proof remain deliberately unexecuted until the owner begins the public/domain transaction. Follow the ordered runbook in `docs/release.md`; do not claim Phase 4 publication complete before the pending CI and those external steps pass.
