...
- [ ] Keep loads, supplies, topology constraints, and acceptance measurements in an evaluator-owned specification. Candidate revisions cannot lower thresholds, replace required parts/models, or remove tests. Use independent formulas/reference calculations where applicable so both workflows are not graded only by Kessetsu's own measurement implementation.
  ...
  - [ ] Harden shared evaluator input/evidence boundaries before comparative use: port U2's rejection of content after `.end` to U1, and retain simulator failure evidence as well as successful datasets. U1 currently ignores `.end` during parsing; a later device must not be reconstructed as an active candidate component.
  - [ ] Extend U2 evaluator coverage to other valid non-inverting low-pass topologies and declared generic models before comparative scoring; unsupported evaluator inputs are not evidence of product failure.
  - [ ] Implement and verify independent evaluators for U3–U6, including model adequacy and fixed topology/testbench integrity; retain acquisition/unsupported outcomes for U6.
