...
- [x] Temiz Windows/Linux/macOS ortamında kurulum → ilk compile → ilk gerçek simulation → assertion smoke testini otomatikleştir. _GitHub run `31727439405`: audit ile Windows x86-64, Linux x86-64, macOS Intel ve Apple Silicon paketleme/checksum/clean-directory gerçek Ngspice 12/12 smoke job'larının tamamı PASS._
...
**İlk remote yayın kapısı kanıtı (2026-08-13):** Billing engeli kalktı; `31727439405` ve provenance düzeltmesini içeren `31729086630` numaralı dört-OS release matrisleri audit dahil bütünüyle PASS verdi. Ubuntu CI run `31727404107`, PATH'ten bulunan Ngspice çalışabildiği halde provenance yolunun absolute olmaması nedeniyle agent benchmark'ında fail ederek gerçek bir taşınabilirlik kusuru yakaladı; bundled/explicit/PATH discovery canonical absolute path'e geçirildi. Follow-up `31729070656` path/agent benchmark'ını geçti ve sonraki model fixture'ında Ubuntu Ngspice 39'un `Cgd`/`Cgs` PMOS parametrelerini reddettiğini gösterdi. `NLANG_PMOS_V1@1.0.1`, explicit portable `MOS1` parametrelerine alındı; model version/directive ve ayrıntılı simulator log regression testiyle yerel canonical iterasyon kapısı PASS. Son Ubuntu full-CI kanıtı bekleniyor.
...
- [x] Desteklenen platformlarda CLI kurulumu ve ilk simülasyon temiz makine release smoke testinden geçer. _Run `31727439405`: dört target PASS; her paket manifest/checksum üretip temiz dizinde gerçek Ngspice ile power-amplifier 12/12 doğruladı._
...
- [ ] Canonical kalite kapısı, browser E2E, schematic connectivity/visual corpus, benchmark parity ve release artifact doğrulamaları geçer. _Yerel canonical tam kapı iki kez PASS; dört-OS artifact matrisi `31727439405` PASS. PATH provenance düzeltmesinin follow-up Ubuntu full-CI kanıtı bekleniyor._
