...
- [ ] Keep loads, supplies, topology constraints, and acceptance measurements in an evaluator-owned specification. Candidate revisions cannot lower thresholds, replace required parts/models, or remove tests. Use independent formulas/reference calculations where applicable so both workflows are not graded only by Kessetsu's own measurement implementation.
  ...
  - [ ] Implement and verify independent evaluators for U5–U6, including model adequacy and fixed topology/testbench integrity; retain acquisition/unsupported outcomes for U6.
...
- [x] Restore and rerun the current full verification gate before release. _Latest run 2026-09-11: full canonical `scripts/verify.ps1` passed, including Rust/WASM, Web lint/10 unit tests/build, 15 Chromium E2E, dependency/license/artifact audits, replay, 24 independent U1–U4 evaluator tests, packaging, and EDA smoke. Npm audit reports zero vulnerabilities. Accepted RustSec maintenance warnings and documented KiCad ERC/export warnings remain; this is not a claim of warning-free EDA interoperability. Rerun against the final release candidate._
...
The owner accepted this direction on 2026-09-10 and authorized developer-led execution. Six task specifications, U1, and a bounded U2 evaluator now exist; U2 topology/model coverage remains open. Next harden the recorded evaluator boundary issues, broaden U2 coverage, then implement U3–U6 and address model-adequacy blockers. Comparative runs and owner-facing interface/release closure are last, per the owner's ordering decision. Historical gates remain recorded; they do not substitute for new design evidence. Public release still requires the integrated Phase 4 gates and explicit owner authorization.
