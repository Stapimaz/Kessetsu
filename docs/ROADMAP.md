...
3. [ ] Activate GitHub Pages/custom-domain DNS and HTTPS for `kessetsu.com`, then verify the real production Web Hub, headers, MIME types, CSP, telemetry boundary, and rollback path. _DNS verification began 2026-09-16._
...
- [ ] Verify a real production Web Hub deployment, including cache headers, WASM/Worker MIME types, CSP, telemetry boundary, and rollback. _Workflow, `/Kessetsu/` content-hash audit, CSP, zero-telemetry policy, and tag/ref rollback are prepared. The repository is public and custom-domain DNS verification is in progress; production deployment has not yet been asserted._
...
- [ ] Create the final release tag/changelog/migration record and publish the verified Web Hub only after every remaining acceptance criterion passes. _`CHANGELOG.md`, migration notes, release workflow, AGPL-3.0-only text, commercial-license notice, Corresponding Source link, and contributor boundary are prepared. Repository visibility changed on 2026-09-16; do not create the immutable tag until history, production, DNS/HTTPS, and release-matrix verification are complete._
...
**4.7 — Verify the production Web Hub and complete the `v1.0.0` release.**

Repository visibility and the authorized history rewrite are complete. Custom-domain DNS verification began on 2026-09-16. The active operation is to deploy and verify the real production Web Hub, including headers, MIME types, CSP, HTTPS, telemetry boundary, and rollback. After that, run the four-platform release workflow and create the immutable `v1.0.0` tag/GitHub Release. Phase 5 remains blocked until those Phase 4 release gates close.
