...
- [ ] Run a clean newcomer journey across landing page, Web editor, packaged CLI, assertions, simulation, and one EDA export; then run the full canonical gate, require green remote Linux CI, and update only the evidence-backed release record. Publication remains owner-controlled. _Local portion completed 2026-09-15: the canonical gate passed end to end; Chromium passed 19 browser scenarios, Firefox passed its fallback document lifecycle with the native-handle case intentionally skipped, and the packaged newcomer smoke passed `check`, 5/5 inline assertions, SVG, KiCad, and 5/5 SHA-256-pinned external requirements. The packaged power-amplifier path passed 12/12 assertions and installed KiCad/LTspice smoke passed for RC, gain-stage, and power-amplifier designs. Awaiting the candidate commit's remote Linux result before closure._
...
**4.6P — Require green remote Linux CI for the locally verified release candidate.**

The accepted visual editor remains the baseline. The Linux geometry failure, false-success paths, analysis ambiguity, external requirement ownership, public examples, editor language help, packaged newcomer journey, and full local canonical gate are closed. The only remaining 4.6P gate is remote Linux CI for the exact candidate commit. Repository visibility, DNS/Pages custom-domain activation, the final tag, GitHub Release, production deployment verification, and rollback proof remain deliberately unexecuted until the owner begins the public/domain transaction.
