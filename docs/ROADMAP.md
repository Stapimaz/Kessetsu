...
- [ ] Compare the same identified model and agent harness with Kessetsu versus direct Ngspice plus scripts and the relevant EDA workflow. Give both equal specifications, tool access, model data, and run budgets; allow baseline automation. Start with one accessible model, repeat each task at least three times, then check whether conclusions transfer to another available model. Record unavailable access rather than inventing runs. _U1–U4 checkpoints completed 2026-09-12/13 with `gpt-5.6-sol`/medium; both arms passed 3/3 electrical evaluations in every family. U1 and U3 favored Kessetsu elapsed time; U2 and U4 favored direct Ngspice. Kessetsu established repeatable automatic artifact/interoperability value: 3/3 full U1 sets, 2/3 U2 sets, and 3/3 U3/U4 sets, with all generated editable exports opening in their target applications. Direct produced hand-authored SVGs in U1 and one U4 attempt, with text-only descriptions otherwise and no editable EDA exports. Evidence: `docs/evals/unseen-design-u1-comparison-2026-09-12.md`, `docs/evals/unseen-design-u2-comparison-2026-09-12.md`, `docs/evals/unseen-design-u3-comparison-2026-09-12.md`, and `docs/evals/unseen-design-u4-comparison-2026-09-13.md`. Continue through U5–U6 before closing the gate._
...
**4.6V — Extend the fresh-context comparison to U5.**

U1–U4 are complete with both arms at 3/3 electrical success in each family and repeatable Kessetsu artifact/interoperability benefit. Extend the isolated runner and equivalent arm documentation to frozen U5 without exposing evaluator code or prior candidates. Keep `gpt-5.6-sol` at medium reasoning, distribute the exact frozen generic op-amp and power-BJT models, and preserve the evaluator's fixed-load amplifier topology plus explicit power-accounting limitations. Run one paired attempt first, inspect quota/evidence integrity, then complete three attempts per arm if sound. Preserve every failure and version any post-candidate protocol correction. Owner-facing interface review and release closure remain after the comparison.
