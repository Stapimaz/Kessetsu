...
- [x] Replace the provisional uppercase/icon identity with an owner-selected production wordmark and favicon before publication. _Completed 2026-09-15: the selected lowercase wordmark uses a short signal line and terminal node with the requested tighter spacing; a 2.2 KiB self-hosted Inter subset keeps the mark stable across platforms, its OFL license ships in Web and CLI notices, the generic Vite favicon is gone, and fresh desktop/mobile/workspace/power-amplifier renders were inspected. Branding E2E, performance, runtime-license, deployment, the full canonical gate, and remote Linux CI run `34904319476` passed on commit `cca5123`._
...
The private v1.0.0 candidate is complete and branded final-candidate commit `cca5123` passed both the local canonical gate and remote Linux CI run `34904319476`. Repository visibility, DNS/Pages custom-domain activation, the final tag, GitHub Release, production deployment verification, and rollback proof remain deliberately unexecuted until the owner begins the public/domain transaction. Follow the ordered runbook in `docs/release.md`; do not claim Phase 4 publication complete before those external steps pass.
