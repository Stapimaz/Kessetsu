...
The guide/installers have passed focused local and four-platform CI gates. Finish the live website deployment and verify `/install/`, both installer endpoints, and the downloadable newcomer example. Installers/guide evolve independently of unchanged `1.0.0` binaries; package sources now retain offline installer-source links, confirmed by focused Windows packaging/link validation.
