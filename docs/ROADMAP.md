...
- [x] Replace the provisional uppercase/icon identity with an owner-selected production wordmark and favicon before publication. _Completed 2026-09-15: the selected lowercase wordmark uses a short signal line and terminal node with the requested tighter spacing; a 2.2 KiB self-hosted Inter subset keeps the mark stable across platforms, its OFL license ships in Web and CLI notices, the generic Vite favicon is gone, and fresh desktop/mobile/workspace/power-amplifier renders were inspected. Branding E2E, performance, runtime-license, deployment, and the full canonical gate passed._
...
- [x] Stabilize the release CI before publication: install the lockfile-defined Playwright package before its matching browser runtime, eliminate the Linux fake-simulator `ETXTBSY` race, and require a green remote run from the final candidate commit. _Completed 2026-09-14: CI installs the lockfile-defined Web dependencies before the matching Playwright browser; the simulator runner retries only Linux `ETXTBSY` launch races with bounded fail-fast behavior preserved elsewhere; remote Linux run `34871602877` first proved the repair, and final-candidate commit `2e2012d` passed run `34873765006` including all 17 Chromium scenarios._
- [x] Complete the Web document lifecycle: new/open/save `.kess`, versioned local draft recovery, explicit document naming, and name-derived source/export filenames without server storage. _Completed 2026-09-14: `File` now owns guarded New/Open/Save/Rename flows; local files and every Core export use a deterministic name-derived filename; stale share fragments are left when editing or replacing their source; and a bounded `kessetsu.web-draft.v1` record restores unsaved browser work without an account or upload. Contract unit coverage and all 17 Chromium E2E passed; the revised desktop workspace screenshot was inspected._
...
- [x] Restore and rerun the current full verification gate before release. _Final-candidate evidence 2026-09-14: clean commit `2e2012d` passed the full canonical `scripts/verify.ps1` locally and remote Linux CI run `34873765006`. Coverage included version/release-note contracts, Rust/WASM, Web lint/13 unit tests/build, payload budgets, runtime/deployment audits, all 17 Chromium E2E, dependency/license/artifact audits, replay and evaluator contracts, the v1.0.0 Windows package plus clean real-Ngspice 12/12 simulation smoke, and installed KiCad/LTspice smoke. The optional user-owned TI OPA197 executions were not bound in this shell and skipped; the earlier exact-hash run and recorded U6 follow-up remain preserved. Npm audit reports zero vulnerabilities. Two documented transitive RustSec maintenance warnings and the characterized embedded-symbol KiCad warnings remain; this is not a claim of warning-free EDA interoperability or hardware validation._
...
The private v1.0.0 candidate is complete and final-candidate commit `2e2012d` passed both the local canonical gate and remote Linux CI run `34873765006`. Repository visibility, DNS/Pages custom-domain activation, the final tag, GitHub Release, production deployment verification, and rollback proof remain deliberately unexecuted until the owner begins the public/domain transaction. Follow the ordered runbook in `docs/release.md`; do not claim Phase 4 publication complete before those external steps pass.
