...
The private v1.0.0 candidate includes the completed owner-facing workspace review: capability-specific document saving, a project-only global toolbar, and Simulation-owned Run/Cancel behavior. The full canonical gate passed locally on 2026-09-15 with 16 Web unit tests, 19 browser scenarios across Chromium and Firefox, zero npm vulnerabilities, and the two already documented informational RustSec maintenance warnings. The latest remote gate must pass before publication. Repository visibility, DNS/Pages custom-domain activation, the final tag, GitHub Release, production deployment verification, and rollback proof remain deliberately unexecuted until the owner begins the public/domain transaction. Follow the ordered runbook in `docs/release.md`; do not claim Phase 4 publication complete before the latest CI and those external steps pass.
