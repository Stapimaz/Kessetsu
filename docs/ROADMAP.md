...
2. [ ] Finish the authorized generated-output history rewrite and verify rewritten `main` through a fresh public clone and Linux CI.
...
- [ ] Purge accidentally committed build outputs from Git history, then verify a fresh public clone and force-push the rewritten `main`. _Owner authorization was received on 2026-09-16. A complete verified bundle backup was created before filtering. Local rewritten history contains none of the former `core/target/**`, `core/out.txt`, or generated `examples/*.spice` paths and its packed Git object store fell to 17.29 MiB. Full verification, guarded force-push, remote Linux CI, and a fresh public-clone audit remain before closure._
...
**4.7 — Complete the in-progress public release transaction.**

The repository became public and custom-domain DNS verification began on 2026-09-16. The active operation is the authorized history rewrite that removes previously committed build outputs; it must pass full local verification, guarded force-push, fresh public-clone inspection, and remote Linux CI. After that, verify the real production Web Hub, headers, MIME types, CSP, HTTPS, telemetry boundary, and rollback; then run the four-platform release workflow and create the immutable `v1.0.0` tag/GitHub Release. Phase 5 remains blocked until those Phase 4 release gates close.
