...
>
> Active milestone: **Post-release prioritization (Phase 5.1 complete)**
>
> Previous milestone: **Phase 5.1 — Simple CLI installation and onboarding (complete, 2026-09-16)**
...
The repository remained private during integrated development and became public on 2026-09-16 after the integrated product gates passed. The CLI/agent surface, simulation and assertion runtime, Web Hub, professional schematics, and target EDA exports must satisfy their acceptance gates together before the immutable `v1.0.0` release. There is no external-user deadline driving a premature tag or production claim.
...
**Commit identity note — 2026-09-16:** Historical implementation references below were remapped to their source-equivalent rewritten commits after generated build outputs were purged. The older CI run IDs retain their original pre-rewrite SHA; they did not execute the new commit identities. Fresh rewritten-history proof is recorded separately in 4.7 (`6978ca9`, run `35026300501`).
...
**Remote evidence:** CI run `31282475618` passed on rewritten commit `bf8e3c0` on 2026-08-09.
...
- [x] SQ-9 — Lock accepted scorecards and deterministic hashes; pass full local and remote verification. _Accepted rewritten commit `dbb02cf`; remote CI run `31850078955`._
...
- [x] Restore a green Linux candidate by making the accepted Export/Share control geometry independent of platform font metrics; retain exact browser coverage rather than weakening the visual contract. _Completed 2026-09-15: both controls use fixed 76 × 30 px geometry with bounded internal padding; the exact browser assertion remains intact. Full local verification passed and remote Linux CI run `34989738740` passed on rewritten commit `74b71e8`._
...
- [x] Complete final public-repository hygiene: organize active documentation by audience, remove completed one-time plans from the active tree, verify English/current claims and local links, audit tracked files and high-confidence credential patterns, and pass canonical local plus remote Linux verification. _Completed 2026-09-16: the docs root now contains only its index and the architecture/roadmap contracts; guides, references, maintainer operations, decisions, evidence, and assets have explicit homes. The completed identity-migration plan moved to Git history, all 45 Markdown files passed local-link audit, active text is English with no former-brand/path residue, and high-confidence credential scans found no matches. The full canonical gate passed, including packaged documentation/newcomer and installed EDA smoke; exact rewritten cleanup commit `d0c55d4` passed remote Linux CI run `35021980964`._
- [x] Purge accidentally committed build outputs from Git history, then verify a fresh public clone and force-push the rewritten `main`. _Completed 2026-09-16 with owner authorization. A complete verified bundle backup was created before filtering. Rewritten history contains none of the former `core/target/**`, `core/out.txt`, or generated `examples/*.spice` paths; `git fsck` passed; and the local packed object store fell to 17.29 MiB. The guarded force-push replaced only unchanged remote `main`. A full public clone at exact commit `6978ca9` passed history/path/fsck/worktree inspection with a 17.54 MiB pack, the full canonical gate passed locally, and remote Linux CI run `35026300501` passed on the rewritten head._
- [x] Stabilize the release CI before publication: install the lockfile-defined Playwright package before its matching browser runtime, eliminate the Linux fake-simulator `ETXTBSY` race, and require a green remote run from the final candidate commit. _Completed 2026-09-14 and reconfirmed 2026-09-15: CI installs the lockfile-defined Web dependencies before the matching Playwright browser; the simulator runner retries only Linux `ETXTBSY` launch races with bounded fail-fast behavior preserved elsewhere; remote Linux run `34871602877` first proved the repair, rewritten commit `460990a` passed run `34873765006`, and the branded rewritten candidate `4339bd8` passed run `34904319476` including all 17 Chromium scenarios._
...
- [x] Automate clean-environment install → compile → real simulation → assertion smoke tests on all four targets. _Initial matrix run `31730843184` passed. The final `1.0.0` candidate `de3ff7c` then passed run `35077825713` on 2026-09-16: Windows x86-64, Linux x86-64, macOS Intel, and macOS Apple Silicon each passed package/version/provenance checks, newcomer `check`, 5/5 inline assertions, SVG/KiCad export, 5/5 hash-pinned external requirements, and real 12/12 power-amplifier simulation._
- [x] Verify a real production Web Hub deployment, including cache headers, WASM/Worker MIME types, CSP, telemetry boundary, and rollback. _Completed 2026-09-16: initial production run `35027497990` deployed `e6659d8`; rollback run `35077405470` explicitly checked out and deployed verified `6978ca9`; restore run `35077915156` deployed final candidate `de3ff7c`. All eight live Chromium scenarios passed after the initial deployment, rollback, and restore. Actual origin checks prove HTTPS/certificate enforcement and HTTP redirect, GitHub-controlled `max-age=600` cache headers, correct JS/CSS/Worker/WASM MIME types, content-hashed assets and the declared performance manifest, restrictive meta CSP, and same-origin GET-only browser traffic without project upload or telemetry. Fresh desktop/mobile/amplifier PNGs were visually inspected; rollback uses immutable source without moving a tag._
  - [x] Add and run a repeatable live-domain browser smoke, isolated from the local preview and canonical CI; retain screenshots/failure traces and verify the complex benchmark, simulation, all exports, and sharing on the actual deployed source. _Completed 2026-09-16: `npm run test:production` passed all eight Chromium scenarios against the real `https://kessetsu.com/` deployment from `e6659d8`. Checks include native/browser benchmark and export parity, Worker simulation/cancellation, named sharing, HTTPS redirect, cached JS/CSS/Worker/WASM MIME headers and manifest lengths, meta CSP, and same-origin GET-only runtime requests with no upload/telemetry. Fresh desktop/mobile/amplifier PNGs were visually reviewed. The new release-only harness also passed the full canonical local gate without affecting the local-preview suite._
...
- [x] Add per-user PowerShell/POSIX installers: supported OS/architecture detection, official release selection, archive and executable hash/version validation, managed install ownership, safe update, PATH setup without administrator privileges, and actionable failures. Do not silently run privileged simulator package installation. _Completed 2026-09-16: managed version directories and atomic launcher/link replacement retain previous working bundles; Windows modifies only user/process PATH, POSIX configures bash/zsh/profile or fish. Exact source `db75d4e` passed real install/update smoke on all four platforms in run `35087272697`, using immutable published `1.0.0` packages. No administrator installation, telemetry, model API calls, or binary retagging was performed._
- [x] Cover successful install/update, corrupt downloads, unsupported platforms, existing/unowned files, and PATH behavior with focused installer contracts and supported-platform smoke. Review desktop/mobile installation-page screenshots and navigation/copy behavior. _Completed 2026-09-16: isolated Windows contracts passed locally with mocked user-PATH writes; POSIX synthetic contracts passed on Linux and both macOS runners (intentionally skipped on the local Windows host). All four native installer jobs in `35087272697` passed default per-user install and repeated update, version/newcomer checks; Windows also passed actual packaged simulation. POSIX contracts cover archive/executable corruption preserving the previous link, unsupported architecture, unmanaged directories, and unrelated executable refusal. Two focused Chromium UI scenarios and desktop/mobile visual review passed. Future canonical verification includes installer contracts; no full engine test suite was repeated for this distribution-only change._
- [x] Update public installation/reference/maintainer guidance and changelog; publish the verified distribution update without moving `v1.0.0`, then check the live installation endpoints and public download journey. _Completed 2026-09-16: README and maintainer guidance document the recommended commands, manual fallback, simulator/ownership/security boundaries, and isolated tests; `[Unreleased]` records the independently evolving distribution tools. A focused Windows package/link check passed after both installer sources were included for offline links. Pages run `35087418772` deployed verified website source `db75d4e`; both live installation UI scenarios passed (2/2), `/install/` returned HTTP 200 with correct canonical metadata, and the deployed PS/POSIX installers plus RC example matched local source SHA-256 byte-for-byte. Published `v1.0.0` still points to `de3ff7c`; no CLI binaries were rebuilt or retagged for the website deployment, and no full engine suite or documentation-only remote CI wait was repeated._
...
**5.1 closure — 2026-09-16:** Guided installation is live at [kessetsu.com/install/](https://kessetsu.com/install/). All four supported native platforms passed installer smoke, negative contracts protect the previous installation and unrelated data, desktop/mobile UI review passed, and the exact deployed installer/example bytes are verified. The immutable CLI release remains `1.0.0`; installation tooling is distributed independently from its binaries.
...
**Post-installation review — choose the next bounded Phase 5 milestone.**
...
The owner-selected installation milestone 5.1 is complete: four-platform installer run `35087272697`, deployment `35087418772`, live UI 2/2, and exact deployed-source checks all passed. No installation publishing blocker remains. Select the next bounded milestone from real newcomer feedback and the prioritized model/interchange, verification, and onboarding opportunities above; implementation has not started. Keep CLI and no-account browser use free, revenue hypotheses demand-led, and future binary releases immutable and SemVer-versioned. Do not implement the entire long-term list or treat regression PASS as a market-success claim.
