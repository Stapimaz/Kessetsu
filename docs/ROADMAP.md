...
- [ ] Restore a green Linux candidate by making the accepted Export/Share control geometry independent of platform font metrics; retain exact browser coverage rather than weakening the visual contract.
...
- [ ] Specify the first immutable-requirements workflow for agent use without bypassing Circuit IR. Decide whether it belongs in `1.0.0` only after a versioned contract, threat boundary, CLI UX, and backward-compatibility review; do not let an agent-controlled design silently redefine evaluator-owned requirements.
- [ ] Curate the shipped example set and newcomer path. Release examples must have intentional names and English commentary; intentionally invalid fixtures stay in the test corpus; README commands must distinguish `check`, `simulate`, and assertion-backed `test` truthfully.
- [ ] Complete practical editor language assistance for the public grammar: all declaration forms, analyses, supported measurement functions, context-appropriate snippets, and useful hover text. `New Circuit` must start from a valid, instructive minimal document rather than a nearly empty stub.
- [ ] Run a clean newcomer journey across landing page, Web editor, packaged CLI, assertions, simulation, and one EDA export; then run the full canonical gate, require green remote Linux CI, and update only the evidence-backed release record. Publication remains owner-controlled.
...
**4.6P — Restore green Linux UI geometry, then close verification-language false-success paths.**

The accepted visual editor remains the baseline. The latest remote run exposed only a platform-font geometry mismatch, but the release audit also found two real trust defects: `kess test` can currently report success with zero assertions, and unknown assertion metrics pass `check` before failing at runtime. Close the ordered 4.6P tasks and record their evidence before returning to deployment/publication. Repository visibility, DNS/Pages custom-domain activation, the final tag, GitHub Release, production deployment verification, and rollback proof remain deliberately unexecuted until the owner begins the public/domain transaction.
