...
- [x] Restore and rerun the current full verification gate before release. _Latest run 2026-09-14: full canonical `scripts/verify.ps1` passed with the exact hash-verified, user-supplied TI OPA197 path enabled, including Rust/WASM, Web lint/10 unit tests/build, 16 Chromium E2E, dependency/license/artifact audits, replay, all required independent U1–U6 evaluator contracts plus two verification-scope contracts, packaging, clean-artifact simulation, and installed KiCad/LTspice smoke. Npm audit reports zero vulnerabilities. Accepted RustSec maintenance warnings and documented KiCad ERC/export warnings remain; this is not a claim of warning-free EDA interoperability. Rerun against the final release candidate._
...
**4.6V — Run the versioned U6 external-model follow-up.**

The typed external-subcircuit contract, native implementation, explicit Web boundary, adversarial coverage, exact local OPA197 integration, and full canonical verification are complete. Preserve the original three unsupported Kessetsu attempts as immutable evidence. Add a versioned follow-up prompt/evaluator path that gives each fresh agent the same user-owned source-relative model and typed declaration contract, then run three new Kessetsu attempts with the authorized `gpt-5.6-sol`/medium Codex CLI. Close 4.6V only if the evaluator accepts the exact model and the resulting visual/editable artifacts retain truthful provenance without containing manufacturer model text.
