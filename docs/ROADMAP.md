...
- [ ] Keep loads, supplies, topology constraints, and acceptance measurements in an evaluator-owned specification. Candidate revisions cannot lower thresholds, replace required parts/models, or remove tests. Use independent formulas/reference calculations where applicable so both workflows are not graded only by Kessetsu's own measurement implementation.
  ...
  - [x] Implement and verify U6 manufacturer-model acquisition/integrity/capability evaluation. _Completed 2026-09-11: exact local-file hash and five-pin OPA197 identity, fixed topology/sources/load, independent AC/transient measurements, no generic fallback, and acquisition/capability error records. TI `SBOMA34D` Rev. D / Final 1.3 was acquired from the official product page for a local probe; hashes and use boundary are recorded in `docs/evals/opa197-model-probe-2026-09-11.md`, while copyrighted model text is neither committed nor emitted in evidence. Bundled Ngspice 46 fails on the authentic PSpice `IF` function even in `ngbehavior=ps`; this is a capability result, not an electrical failure. Synthetic contract tests and the optional official local probe passed in their intended outcomes; full canonical verification passed with the copyrighted-model probe skipped when the local file is absent._
  - [ ] Resolve the U6 model/interchange blocker before comparative runs: obtain a clearly redistributable authentic compatible model, implement and verify a deterministic licensed PSpice-to-Ngspice adapter, or add a simulator backend that executes the exact official artifact. Preserve the source file/hash and model notices; do not silently rewrite it, bundle it under incompatible terms, or substitute the generic template.
...
- [x] Restore and rerun the current full verification gate before release. _Latest run 2026-09-11: full canonical `scripts/verify.ps1` passed, including Rust/WASM, Web lint/10 unit tests/build, 15 Chromium E2E, dependency/license/artifact audits, replay, 34 independent U1–U6 evaluator tests (33 pass; one intentional skip for the unbundled copyrighted TI model), packaging, and EDA smoke. The separate official-model local probe passed by producing the expected capability error. Npm audit reports zero vulnerabilities. Accepted RustSec maintenance warnings and documented KiCad ERC/export warnings remain; this is not a claim of warning-free EDA interoperability. Rerun against the final release candidate._
...
**4.6V — Resolve the authentic manufacturer-model interoperability blocker demonstrated by U6.**

The owner accepted this direction on 2026-09-10 and authorized developer-led execution. Six frozen task specifications and bounded, canonically verified U1–U6 evaluator paths now exist. The authentic TI OPAx197 Rev. D artifact is integrity-verified but not executable by bundled Ngspice 46, and its redistribution terms are not treated as compatible with Kessetsu distribution. Next scope and resolve that model/interchange blocker without weakening provenance or substituting a generic model. Comparative runs and owner-facing interface/release closure remain last, per the owner's ordering decision. Historical gates do not substitute for new design evidence. Public release still requires the integrated Phase 4 gates and explicit owner authorization.
