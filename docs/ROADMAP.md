...
- [ ] Compare the same identified model and agent harness with Kessetsu versus direct Ngspice plus scripts and the relevant EDA workflow. Give both equal specifications, tool access, model data, and run budgets; allow baseline automation. Start with one accessible model, repeat each task at least three times, then check whether conclusions transfer to another available model. Record unavailable access rather than inventing runs. _U1–U5 checkpoints completed 2026-09-12/13 with `gpt-5.6-sol`/medium; both arms passed 3/3 electrical evaluations in every family. U1, U3, and U5 favored Kessetsu elapsed time; U2 and U4 favored direct Ngspice. Kessetsu established repeatable automatic artifact/interoperability value: 3/3 full U1 sets, 2/3 U2 sets, and 3/3 U3–U5 sets, with all generated editable exports opening in their target applications. Direct produced hand-authored SVGs in U1 and one U4 attempt, with text-only descriptions otherwise and no editable EDA exports. Evidence: `docs/evals/unseen-design-u1-comparison-2026-09-12.md` through `docs/evals/unseen-design-u5-comparison-2026-09-13.md`. Continue through U6 before closing the gate._
...
- [ ] Save prompts, exact available model identifiers/settings, sources, tool traces, failures, human interventions, elapsed time, usage/cost when available, and final artifacts. Report successful designs, false passes, unsupported cases, and artifact usability separately. Existing fixture replay remains a regression test. _U1–U5 records are complete locally under `.artifacts/agent-comparison-v1/`; their committed reports record scope, hashes, usage, corrections/exclusions, and artifact usability. Repeat for U6 before checking this item._
- [ ] Review fresh schematic PNGs and open editable exports from successful unseen tasks; a corpus hash alone cannot establish readable new layouts. _U1 completed 2026-09-12: three fresh Kessetsu PNGs and all editable exports passed review. U2 completed 2026-09-12: both generated Kessetsu PNGs and editable exports passed review; one Kessetsu run omitted exports and is recorded as 2/3 artifact delivery. U3 completed 2026-09-12: three PNGs passed visual review, including BJT arrows; all three KiCad/LTspice exports opened/netlisted with all ten references and zero KiCad ERC errors beyond ten known `lib_symbol_issues` warnings. U4 completed 2026-09-13: three clean Kessetsu PNGs passed visual review; all three editable exports opened/netlisted with all four references and zero KiCad ERC errors beyond four known `lib_symbol_issues` warnings. U5 completed 2026-09-13: three clean Kessetsu power-amplifier PNGs passed visual review; all three editable exports opened/netlisted with all 11 references and zero KiCad ERC errors beyond 11 known `lib_symbol_issues` warnings. Direct U2/U3/U5 and two U4 attempts produced text descriptions; the remaining U4 direct SVG was structurally valid but not rendered, so no visual-quality claim is made. Continue for U6._
...
- [x] Restore and rerun the current full verification gate before release. _Latest run 2026-09-13: full canonical `scripts/verify.ps1` passed after the U5 comparison checkpoint, including Rust/WASM, Web lint/10 unit tests/build, 15 Chromium E2E, dependency/license/artifact audits, replay, all required independent U1–U6 evaluator contracts plus two verification-scope contracts, packaging, clean-artifact simulation, and installed KiCad/LTspice smoke. The optional local U6 manufacturer-model probe was skipped because the user-supplied model path was not set; its exact-model path had passed on 2026-09-11. Npm audit reports zero vulnerabilities. Accepted RustSec maintenance warnings and documented KiCad ERC/export warnings remain; this is not a claim of warning-free EDA interoperability. Rerun against the final release candidate._
...
**4.6V — Extend the fresh-context comparison to U6.**

U1–U5 are complete with both arms at 3/3 electrical success in each family and repeatable Kessetsu artifact/interoperability benefit. Extend the isolated runner and equivalent arm documentation to frozen U6 without exposing evaluator code or prior candidates. Keep `gpt-5.6-sol` at medium reasoning and distribute the same exact, owner-supplied TI OPA197 model plus its frozen provenance/hash to both arms without committing or redistributing the copyrighted model. Preserve the evaluator's acquisition/capability failure semantics and PSpice compatibility boundary. Run one paired attempt first, inspect quota/evidence integrity, then complete three attempts per arm if sound. Preserve every failure and version any post-candidate protocol correction. Owner-facing interface review and release closure remain after the comparison.
