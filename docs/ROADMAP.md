...
- [ ] Specify a typed external-subcircuit reference contract for Core/CLI: exact content hash, provenance/version/license, declared pin order/component kind, redistribution flag, simulator compatibility mode, deterministic lock data, and fail-closed diagnostics. Raw manufacturer model bodies must not become Kessetsu source or silently degrade to a generic template.
- [ ] Implement the bounded contract through Circuit IR, native compile/simulate, model lock/provenance reporting, schematic metadata, and KiCad/LTspice export without bypassing the shared IR. The source design references a user-owned local model; Kessetsu neither embeds nor redistributes it.
- [ ] Define and test the Web boundary explicitly: safe user-selected model ingestion when the browser runtime supports the required simulator capability, otherwise a precise unsupported result with no server upload or generic fallback.
- [ ] Run tampering, missing-file, changed-hash, pin-order, unsupported-runtime, injection, and exact OPA197 integration tests; rerun full canonical verification.
- [ ] Run a versioned U6 follow-up with three fresh Kessetsu attempts. Preserve the original unsupported result; close 4.6V only if the exact model passes and usable artifacts retain truthful provenance.
...
- [x] Restore and rerun the current full verification gate before release. _Latest run 2026-09-13: full canonical `scripts/verify.ps1` passed after the U6 comparison/decision checkpoint, including Rust/WASM, Web lint/10 unit tests/build, 15 Chromium E2E, dependency/license/artifact audits, replay, all required independent U1–U6 evaluator contracts plus two verification-scope contracts, packaging, clean-artifact simulation, and installed KiCad/LTspice smoke. The canonical run skipped the optional user-supplied U6 model path; a separate same-day run with the exact hash-verified TI model passed all 5/5 U6 tests. Npm audit reports zero vulnerabilities. Accepted RustSec maintenance warnings and documented KiCad ERC/export warnings remain; this is not a claim of warning-free EDA interoperability. Rerun against the final release candidate._
...
**4.6V — Close the evidence-selected external manufacturer-model blocker.**

The first-model U1–U6 comparison and decision record are complete. Kessetsu's repeatable value is automatic trustworthy artifacts/interoperability, while speed is mixed and electrical success matched direct Ngspice on supported tasks. U6 proved that the current typed model boundary cannot represent the exact user-supplied OPA197 subcircuit. Specify the smallest safe external-subcircuit reference that preserves hash/provenance/pins/license/runtime capability in Circuit IR, then implement it through native simulation and truthful exports without embedding or redistributing the model. Define the Web unsupported/upload boundary explicitly, harden the contract against tampering and injection, and run a versioned U6 follow-up plus full canonical verification. Do not begin owner-facing review or release closure until this gate is resolved.
